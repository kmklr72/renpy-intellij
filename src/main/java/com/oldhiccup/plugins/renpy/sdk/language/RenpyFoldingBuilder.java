...
public class RenpyFoldingBuilder extends FoldingBuilderEx implements DumbAware {

  @NotNull
  @Override
  public FoldingDescriptor[] buildFoldRegions(@NotNull PsiElement root, @NotNull Document document, boolean quick) {
    // Initialize the group of folding regions that will expand/collapse together.
    FoldingGroup group = FoldingGroup.newGroup(RenpyAnnotator.RENPY_PREFIX_STR);
    ...
    for (final PsiLiteralExpression literalExpression : literalExpressions) {
      ...
      if (value != null && value.startsWith(RenpyAnnotator.RENPY_PREFIX_STR + RenpyAnnotator.RENPY_SEPARATOR_STR)) {
        ...
        final List<RenpyProperty> properties = RenpyUtil.findProperties(project, key);
  ...
  /**
   * Gets the Renpy Language 'value' string corresponding to the 'key'
   *
   * @param node Node corresponding to PsiLiteralExpression containing a string in the format
   *             RENPY_PREFIX_STR + RENPY_SEPARATOR_STR + Key, where Key is
   *             defined by the Renpy language file.
   */
  @Nullable
  @Override
  public String getPlaceholderText(@NotNull ASTNode node) {
    ...
    if (node.getPsi() instanceof PsiLiteralExpression) {
      ...
      final List<RenpyProperty> properties = RenpyUtil.findProperties(nodeElement.getProject(), key);
